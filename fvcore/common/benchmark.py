...
def timeit(num_iters: int = -1, warmup_iters: int = 0):
    ...
    Args:
        num_iters: number of iterations used to compute the average time (sec)
            required to run the function. If negative, the number of iterations
            is determined dynamically by running the function a few times to
            make sure the estimate is stable.
        warmup_iters: number of iterations used to warm up the function. This
            is useful for functions that exhibit poor performance during the
            first few times they run (due to caches, autotuning, etc).
    Returns:
...
def benchmark(
    ...
    *,
    ...
    Args:
        func: a closure that returns a function for benchmarking, where
            initialization can be done before the function to benchmark.
        bm_name: name of the benchmark to print out, e.g. "BM_UPDATE".
        kwargs_list: a list of argument dict to pass to the function. The
            intput function will be timed separately for each argument dict.
        num_iters: number of iterations to run. Defaults to run until 0.5s.
        warmup_iters: number of iterations used to warm up the function.
