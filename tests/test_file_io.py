...
class TestCloudUtils(unittest.TestCase):
    ...
    def _add_gc_methods_to_file(self, file: Union[IO[str], IO[bytes]]):
        ...
        self.assertEqual(
            inspect.getsource(file.close), inspect.getsource(close_and_upload)
        )
        file._close()
    ...
    def test_maybe_make_directory_doesnt_exist(self):
        self.assertTrue(
            self.gc_pathhandler._maybe_make_directory("./tmp/test/path/test.txt")
        )
    ...
    def test_add_client_to_handler(self):
        self.gc_pathhandler._create_gc_client(
            "/".join([self.gc_default_path, "path/file.txt"])
        )
    ...
    def test_get_requested_gc_bucket(self):
        gc_bucket = self.gc_pathhandler._get_gc_bucket(
            "/".join([self.gc_default_path, "path/file.txt"])
        )
    ...
    def test_get_blob(self):
        gc_blob = self.gc_pathhandler._get_blob(
            "/".join([self.gc_default_path, "path/file.txt"])
        )
    ...
    def test_exist_when_blob_exists(self):
        self.assertTrue(
            self.gc_pathhandler._exists("/".join([self.gc_default_path, ""]))
        )

    @unittest.skipIf(not gc_auth, skip_gc_auth_required_tests_message)
    def test_exist_when_blob_doesnt_exist(self):
        self.assertFalse(
            self.gc_pathhandler._exists(
                "/".join([self.gc_default_path, "doesnt/exist.txt"])
            )
        )

    @unittest.skipIf(not gc_auth, skip_gc_auth_required_tests_message)
    def _gc_local_file_write_and_upload(
        self, file: Union[IO[str], IO[bytes]], message: str
    ):
        gc_blob = self.gc_pathhandler._get_blob(
            "/".join([self.gc_default_path, "path/test.txt"])
        )
        self.gc_pathhandler._decorate_file_with_gc_methods(file, gc_blob)
        file.write(message)
        file.close()
    ...
    def test_open_read_text_file(self):
        file = self.gc_pathhandler._open(
            "/".join([self.gc_default_path, "path/test2.txt"])
        )
    ...
    def test_get_local_path_remote_file_doesnt_exists(self):
        ...
        self.assertTrue(
            os.path.exists(self.gc_pathhandler._get_local_cache_directory(remote_path))
        )
